...
/**
 * Get game configuration from AsyncStorage
 * @returns GameConfig (merged with defaults)
 */
export async function getGameConfig(): Promise<GameConfig> {
  try {
    const saved = await storage.getItem<GameConfig>(CONFIG_STORAGE_KEY);
    if (saved) {
...
/**
 * Reset game configuration (remove from AsyncStorage)
 */
export async function resetGameConfig(): Promise<void> {
  try {
    await storage.removeItem(CONFIG_STORAGE_KEY);
  } catch (error) {
 ...
 * @param config - Optional config (if not provided, will load from storage)
 */
export function getXpRequiredForLevel(level: number, config?: GameConfig): number {
  const gameConfig = config || DEFAULT_CONFIG;
  if (level <= 0) return 0;
  if (level === 1) return gameConfig.baseXpPerLevel;

  // Calculate XP required for a specific level using compound growth
  const growthMultiplier = 1 + (gameConfig.xpGrowthPercentage / 100);
  return Math.floor(gameConfig.baseXpPerLevel * Math.pow(growthMultiplier, level - 1));
}

/**
 * Get total XP needed to reach a specific level from level 0
 * @param level - Target level
 * @param config - Optional config (if not provided, will use DEFAULT_CONFIG)
 */
export function getTotalXpForLevel(level: number, config?: GameConfig): number {
  let totalXp = 0;
  for (let i = 1; i <= level; i++) {
    totalXp += getXpRequiredForLevel(i, config);
  }
  return totalXp;
}
...
export function getLevelFromXp(
  totalXp: number,
  config?: GameConfig
): { level: number; currentXp: number; xpForNextLevel: number } {
  const gameConfig = config || DEFAULT_CONFIG;
  let level = 0;
  let xpAccumulated = 0;

  while (level < gameConfig.maxLevel) {
    const xpNeeded = getXpRequiredForLevel(level + 1, gameConfig);
  ...
  const xpForNextLevel = level < gameConfig.maxLevel
    ? getXpRequiredForLevel(level + 1, gameConfig)
    : 0;
