...
async function saveStreakData(data: StreakData): Promise<void> {
  try {
    await storage.setItem(STREAK_STORAGE_KEY, data);
  } catch (error) {
...
// Load streak data from AsyncStorage (internal version)
async function loadStreakDataInternal(): Promise<StreakLoadResult> {
  let freezeUsed = false;
  let freezesUsedCount = 0;
  try {
    const saved = await storage.getItem<StreakData>(STREAK_STORAGE_KEY);
    if (saved) {
      const data = saved;
...
/**
 * Load streak data from AsyncStorage (public version)
 */
export async function loadStreakData(): Promise<StreakData> {
  const result = await loadStreakDataInternal();
  return result.data;
}

/**
 * Record a task completion and update streak
 * @returns Object with isFirstToday, newStreak, and longestStreak
 */
export async function recordTaskCompletion(): Promise<{
...
export async function resetStreak(): Promise<void> {
  try {
    await storage.removeItem(STREAK_STORAGE_KEY);
  } catch (error) {
...
/**
 * Add streak freezes
 * @param count - Number of freezes to add (default: 1)
 * @returns Total number of freezes after adding
 */
export async function addStreakFreeze(count: number = 1): Promise<number> {
  const data = await loadStreakData();
