...
export default function LessonGameScreen({
  ...
  const [timeLeft, setTimeLeft] = useState(60); // Matching time limit
  ...
  useEffect(() => {
    if (gameStatus === 'playing' && matchedCount === totalPairs && totalPairs > 0) {
      setGameStatus('won');
    }
  }, [matchedCount, gameStatus, totalPairs]);
      ...
      <View style={styles.centerContainer}>
        ...
        >
          ...
          <AnimatedTouchable
            ...
            style={[styles.continueButton, styles.continueButtonLose]}
            entering={FadeIn.delay(400).duration(400)}
            activeOpacity={0.9}
          >
            <Text style={styles.continueButtonText}>TOVÁBB</Text>
      ...
      <View style={styles.headerSection}>
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerStatsRow}>
            ...
            <View style={[
              styles.timerBadge,
              timeLeft <= 10 && styles.timerBadgeLow
            ]}>
              <Text style={[
                styles.timerText,
                timeLeft <= 10 && styles.timerTextLow
              ]}>
                {timeLeft}s
              </Text>
            </View>
        ...
        <View style={styles.gameGrid}>
          ...
          <View style={styles.columnWrapper}>
            {leftBoxes.map((box, index) => (
              <AnimatedTouchable
                key={box.id}
                onPress={() => handleLeftClick(box)}
                style={[
                  styles.matchBox,
                  styles.matchBoxLeft,
                  selectedLeft?.id === box.id && styles.matchBoxSelected,
                ]}
          ...
          <View style={styles.columnWrapper}>
            {rightBoxes.map((box, index) => (
              <AnimatedTouchable
                key={box.id}
                onPress={() => handleRightClick(box)}
                style={[
                  styles.matchBox,
                  styles.matchBoxRight,
                  selectedRight?.id === box.id && styles.matchBoxSelected,
                  flashingBox === box.id && styles.matchBoxFlashing,
                ]}
                entering={FadeIn.delay(index * 100).duration(400)}
                activeOpacity={0.8}
              >
